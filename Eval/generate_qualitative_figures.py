#!/usr/bin/env python3
"""
Script to generate qualitative visualizations for the paper.
This is separate from the main evaluation script to avoid 
matplotlib import issues and double visualization.
"""
...
def generate_visualizations(data_dir):
    ...
    output_dir = os.path.join(os.path.dirname(__file__), 'qualitative_output')
    ...
    for scenario_type, frame_id in scenarios.items():
        print(f"\nProcessing {scenario_type} (Frame {frame_id})...")
        try:
            # Load frame data
            ...
            try:
                ...
                print(f"  Warning: LiDAR projected points not found")
            
            # Create visualization
            fig = plt.figure(figsize=(15, 10))
            
            # RGB image with detections
            ax1 = plt.subplot(221)
            ax1.imshow(frame_data['rgb'])
            # Try to load and overlay detection boxes
            try:
                boxes_path = os.path.join(data_dir, 'debug', f"{frame_id}_boxes.npy")
                ...
                print(f"  Warning: Could not load detection boxes: {e}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate qualitative visualizations for the paper")
    parser.add_argument('--data_dir', default='data/fused_output',
                       help='Directory containing evaluation results')
    args = parser.parse_args()
    generate_visualizations(args.data_dir)