...
def load_or_generate_depth_data(frame_rgb, frame_number, figures_dir):
    """Load or generate depth data from various sources."""
    ...
    if os.path.exists(lidar_npz):
        try:
            ...
            if points_2d is not None and depths is not None:
                ...
                depth_data['lidar'] = lidar_depth
                depth_data['lidar_points'] = {'points': points_2d, 'depths': depths}
        except Exception as e:
            print(f"Warning: Could not process LiDAR data: {e}")
    
    # Load fused depth if available
    fused_path = os.path.join(project_root, "data/fused_output/debug", 
                             f"{frame_number}_fused_depth.npy")
    if os.path.exists(fused_path):
        depth_data['fused'] = np.load(fused_path)
...
def create_visualization_figures(frame_number):
    ...
    # Get frame dimensions and load all depth data
    H, W = frame_rgb.shape[:2]
    depth_data = load_or_generate_depth_data(frame_rgb, frame_number, figures_dir)
    ...
    # 2. Monocular depth with boxes
    plt.subplot(142)
    mono_depth = depth_data.get('mono')
    if mono_depth is not None:
        plt.imshow(frame_rgb)
        for box in pred_bboxes:
            x1, y1, x2, y2 = map(int, box[:4])
            cls_idx = int(box[4])
            box_depth = robust_box_depth(mono_depth[y1:y2, x1:x2])
            depth_roi = mono_depth[y1:y2, x1:x2]
    ...
    plt.subplot(143)
    plt.imshow(frame_rgb)
    lidar_depth = depth_data.get('lidar')
    lidar_points = depth_data.get('lidar_points')
        ...
        for box in pred_bboxes:
            ...
            if np.isfinite(box_depth):
                ...
                plt.text(x1, y1-10, f"D:{box_depth:.1f}m", color='white', fontsize=8,
                        bbox=dict(facecolor='black', alpha=0.7, edgecolor='none'))
        if lidar_points:
            plt.scatter(lidar_points['points'][:, 0], lidar_points['points'][:, 1], 
                       c=lidar_points['depths'], cmap='magma', alpha=0.5, s=1)
        plt.colorbar(label='LiDAR Depth (m)')
    plt.title('LiDAR Ground Truth')
    plt.axis('off')
    
    # 4. ECW with multi-source depth
    plt.subplot(144)
    plt.imshow(frame_rgb)
    ...
    for box in pred_bboxes:
        ...
        depths = {}
        for source, depth in depth_data.items():
            if depth is not None:
                depths[source] = robust_box_depth(depth[y1:y2, x1:x2])
        
        # Compute TTC using minimum depth
        if depths:
